...
+ (instancetype) sharedConfig {
    static ACConfiguration *shardConfig = nil;
...
- (instancetype) init {
    ...
    if (self) {
        self.apiClient = nil;
        self.host = @"https://api.artik.cloud/v1.1";
        self.username = @"";
        self.password = @"";
        self.accessToken= @"";
        self.verifySSL = YES;
        ...
        self.mutableDefaultHeaders[@"User-Agent"] = [NSString stringWithFormat:@"Swagger-Codegen/2.0.0/objc (%@; iOS %@; Scale/%0.2f)",[[UIDevice currentDevice] model], [[UIDevice currentDevice] systemVersion], [[UIScreen mainScreen] scale]];
        self.logger = [ACLogger sharedLogger];
    }
    return self;
}
