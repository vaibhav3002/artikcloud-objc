...
NSString *const ACResponseObjectErrorKey = @"ACResponseObject";
...
static NSDictionary * AC__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * AC__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = AC__headerFieldsForResponse(response);
...
- (instancetype)init {
    NSString *baseUrl = [[ACConfiguration sharedConfig] host];
...
- (instancetype)initWithBaseURL:(NSURL *)url {
    ...
    if (self) {
        ...
        self.responseDeserializer = [[ACResponseDeserializer alloc] init];
        self.sanitizer = [[ACSanitizer alloc] init];
...
+(NSNumber*) queueRequest {
    ...
    ACDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        ACDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    } else {
        return NO;
...
- (void) configureCacheReachibility {
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        reachabilityStatus = status;
        ACDebugLog(@"reachability changed to %@",AFStringFromNetworkReachabilityStatus(status));
        [ACApiClient setOfflineState:status == AFNetworkReachabilityStatusUnknown || status == AFNetworkReachabilityStatusNotReachable];
...
- (void) operationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        ACDebugLogResponse(response, responseObject,request,error);
        strongSelf.HTTPResponseHeaders = AC__headerFieldsForResponse(response);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[ACResponseObjectErrorKey] = responseObject;
        }
...
- (void) downloadOperationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString * filename = AC__fileNameForResponse(response);
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [ACJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [ACJSONResponseSerializer serializer];
    } else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = ACPercentEscapedStringFromString(safeString);
    ...
    if (offlineState) {
        ACDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        ACDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        ACDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [ACApiClient queueRequest];
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = ACPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, ACPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[ACQueryParamCollection class]]){
            ACQueryParamCollection * coll = (ACQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = ACPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = ACPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = ACPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (AFSecurityPolicy *) customSecurityPolicy {
    ...
    ACConfiguration *config = [self configuration];
...
- (ACConfiguration*) configuration {
    return [ACConfiguration sharedConfig];
}
