...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        ACDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [ACApiClient nextRequestId];
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                ACDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [ACApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                ACDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [ACApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                ACDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [ACApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                ACDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [ACApiClient setOfflineState:false];
                break;
            default:
                break;
                                                               ...
                                                               success:^(AFHTTPRequestOperation *operation, id response) {
                                                                   ...
                                                                   if ([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if (operation.responseObject) {
                                                                           ...
                                                                           userInfo[ACResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[ACApiClient escape:obj]];
    }];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if ([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [ACApiClient escape:key], [ACApiClient escape:[queryParams valueForKey:key]]]];
            }
                ...
                if ([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [ACApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [ACApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [ACApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[ACQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[ACObject class]]) {
